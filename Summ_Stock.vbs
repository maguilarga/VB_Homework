...
' Loop through all of the worksheets in the active workbook.
    For Each Current_WS In Worksheets
        ...
        ORow = 2
        OpenValue = Range("C2").Value   ' Open value of first stock
        Volume = 0
    
        ' Loop through all rows in the worksheet
        For IRow = 2 To ILastRow
        ' If the next ticker is different from the current ticker, summarize it
            If (Range("A" & IRow + 1).Value <> Range("A" & IRow).Value) Then
            
            ' Ticker name
                Range("J" & ORow).Value = Range("A" & IRow).Value
            
            ' Value Change
                ValChange = Range("F" & IRow).Value - OpenValue
            ...
            ' Percentage of Change
                If OpenValue = 0 Then ' Special case: open Value was 0
                    ...
                    PercChange = 0
                Else
                    PercChange = (Range("F" & IRow).Value / OpenValue) - 1
            ...
            ' Volume
                Volume = Volume + Range("G" & IRow).Value
